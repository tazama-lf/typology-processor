...
import { test, expect, mock, beforeAll, beforeEach, describe, jest } from "bun:test";
...
describe('Logic Service', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(axios, 'post').mockImplementation(async (url: string, data?: any, config?: AxiosRequestConfig) => { return { status: 201 } });
  ...
  describe('Handle Transaction', () => {
    test('should handle successful request, and should have weight of 100', async () => {
    ...
    //   jest.spyOn(axios, 'post').mockImplementationOnce(async (url: string, data?: any, config?: AxiosRequestConfig) => { return { status: 401 } });
    ...
    //   jest.spyOn(axios, 'post').mockImplementationOnce(async (url: string, data?: any, config?: AxiosRequestConfig) => { return { status: 200 } });

    //   await handleTransaction({ transaction: expectedReq, ruleResult: ruleResult03, networkMap });

    //   jest.spyOn(axios, 'post').mockImplementationOnce(async (url: string, data?: any, config?: AxiosRequestConfig) => {
    //     return new Promise((resolve, _reject) => {
    //       resolve({ status: 400 });
    //     })
    //   });
    ...
    //   jest.spyOn(axios, 'post').mockImplementationOnce(async (url: string, data?: any, config?: AxiosRequestConfig) => Promise.reject("Test Failure Path"));
    ...
    //   const errorSpy = jest.spyOn(server, 'handleResponse').mockRejectedValue(new Error('Testing purposes'));
    //   try {
    ...
    //   }
    //   catch {
    //     expect("Error HandlingResponse").toBe("Handled not error out.")
    //   }
