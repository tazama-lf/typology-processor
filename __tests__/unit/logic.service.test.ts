...
let cacheString: RuleResult | string;
let weight: number;
...
describe('Logic Service', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(databaseManager, 'getMembers').mockImplementation((_key: string): Promise<RuleResult[]> => {
      return new Promise<RuleResult[]>((resolve, _reject) => {
        if (typeof cacheString !== 'string') resolve([cacheString]);
      });
    });

    jest.spyOn(databaseManager, 'addOneGetCount').mockImplementation((_key: string, value: RuleResult): Promise<number> => {
